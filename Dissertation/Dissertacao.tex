...
First, the average over 1 period of the three-phase current signals \(I_A\), \(I_B\) and \(I_C\) are calculated with their respectives phase angles.
...
In \cite{Diallo2005}, the authors suggested a technique differentiated for high and low power applications with a view for the special needs of each class. For higher power drives, the authors alleged that the location of the fault becomes crucial, whereas for low power, less than 1 kW, the key point is detecting abnormal conditions, once the power module is of lower cost than a repair.
...
in \cite{BandyopadhyayPrithwiraj16}, 
